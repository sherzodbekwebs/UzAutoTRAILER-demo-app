...
const Configurator = ({ lang, setCurrentPage, setSelectedTruckId }) => {
    ...
    return (
        <div className="bg-white h-auto font-sans select-none text-[#1d1d1b] overflow-x-hidden">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 py-6">
                ...
                <div className="relative">
                    {/* --- STEP 1 --- */}
                    <div className="flex gap-4 md:gap-10 relative">
                        <div className="flex flex-col items-center relative">
                            <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center z-10 transition-all duration-500 
                                ...
                                {selectedCategory ? <Check size={14} /> : <ChevronRight size={18} />}
                            </div>
                            <div className={`w-[2px] absolute top-8 bg-[#9ba4b0]/40 transition-all duration-500 ${selectedCategory ? 'h-full' : 'h-24'}`}></div>
                        </div>

                        <div className="flex-1">
                            <h2 className="text-[12px] md:text-[14px] font-bold text-[#556071] uppercase flex items-center gap-2 md:gap-4 mb-4">
                                <span className="text-blue-100 text-xl md:text-2xl font-light">〈</span> {t.step1}
                            </h2>

                            {/* Responsive Grid for Types */}
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 border border-gray-100 bg-white mb-6 overflow-hidden shadow-sm">
                                {t.types.map((type) => (
                                    <div
                                        key={type.id}
                                        onClick={() => handleCategoryChange(type.id)}
                                        className={`group cursor-pointer flex flex-col items-center justify-center border-r border-b sm:border-b-0 last:border-r-0 border-gray-200 px-2 py-6 md:py-8 transition-all duration-300 relative
                                            ${selectedCategory === type.id ? 'bg-[#f4f7f9]' : 'hover:bg-gray-50'}`}
                                    >
                                        ...
                                        <span className={`text-[7.5px] md:text-[8.5px] font-black text-center uppercase leading-tight px-1 transition-colors duration-300 ${selectedCategory === type.id ? 'text-[#e67e22]' : 'text-[#0054A6]'}`}>
                                            {type.name}
                                        </span>
                    ...
                    <div className={`flex gap-4 md:gap-10 relative transition-all duration-500 ease-in-out overflow-hidden
                        ...
                        <div className="flex flex-col items-center relative">
                            <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center z-10 transition-all duration-500
                                ...
                                {selectedFormula ? <Check size={14} /> : <ChevronRight size={18} />}
                            </div>
                            <div className={`w-[2px] absolute top-8 bottom-0 bg-[#9ba4b0]/40 ${selectedFormula ? 'h-full' : 'h-24'}`}></div>
                        </div>

                        <div className="flex-1">
                            <h2 className="text-[12px] md:text-[14px] font-bold text-[#556071] uppercase flex items-center gap-2 md:gap-4 mb-4 md:mb-6">
                                <span className="text-blue-100 text-xl md:text-2xl font-light">〈</span> {t.step2}
                            </h2>

                            <div className="flex flex-wrap gap-4 md:gap-8 pl-2 md:pl-6 mb-8">
                                {availableFormulas.map((formula) => (
                                    <label key={formula} className="flex items-center gap-3 md:gap-4 cursor-pointer group">
                                        ...
                                        <span className={`text-base md:text-lg font-medium ${selectedFormula === formula ? 'text-[#1d1d1b]' : 'text-gray-400 group-hover:text-gray-600'}`}>{formula}</span>
                                    </label>
                                ))}
                            </div>
                            ...
                            {/* --- RESULTS SECTION --- */}
                            {selectedFormula && (
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 animate-in fade-in duration-700 pb-10">
                                    {/* Configuration Table */}
                                        ...
                                        <div className="space-y-0 border-t border-gray-100">
                                            {finalResults.map(truck => (
                                                <div key={truck.id} onClick={() => handleTruckClick(truck.id)}
                                                    ...
                                                    <span className="text-[12px] md:text-[13px] font-black text-[#0054A6] uppercase underline group-hover:text-orange-600 transition-colors leading-snug">
                                                        {truck.name}
                                                    </span>
                                                </div>
                                            ))}
